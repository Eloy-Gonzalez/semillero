...
function* getUsuariosWorker({ borrado = false} = {}) {
  try {
    yield put({ type: REQUEST_STARTED })

    const res = yield call(getUsuariosServices, {borrado})
    const {data} = res
    if(data.rows) {
      yield put({ type: SET_USUARIOS, payload: data})
    } else {
...
function* requestWatcher() {
  yield takeLatest(GET_USUARIOS, getUsuariosWorker)
  // yield takeLatest(UPDATE_USUARIO, updateVideoWorker)
  // yield takeLatest(DELETE_USUARIO, deleteVideoWorker)
  // yield takeLatest(RESTORE_USUARIO, restoreVideoWorker)
}
