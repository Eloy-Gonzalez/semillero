...
// @ActionsTypes
import {UPDATE_USUARIO, DELETE_USUARIO} from 'state/admin/usuarios/usuarios.actionsTypes'
import {openDialogConfirm} from 'state/app/app.actions'

function DetailsUsuario({ data, loading=false }) {
	...
	const onSubmit = React.useCallback( (data, actions) => {
		...
		dispatch({type: UPDATE_USUARIO, payload})
	...
	const onDelete = useCallback((id, version) => {
		...
		dispatch(openDialogConfirm({
			...
			description: "¿Seguro de Eliminar este usuario de forma permantene?",
			onConfirm: () => dispatch({ type: DELETE_USUARIO, payload})
		}))
	}, [dispatch])
...
export default React.memo(DetailsUsuario)